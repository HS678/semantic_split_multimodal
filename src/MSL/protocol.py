...
def add_experiment_args(parser: argparse.ArgumentParser, *, baseline: bool = False, include_seed: bool = True):
    ...
    parser.add_argument("--pretrain-epochs", type=int, help="Client pretraining epochs.")
    parser.add_argument("--pretrain-lr", type=float, help="Client pretraining learning rate.")
    parser.add_argument("--fingerprint-type", choices=["encoder", "signal", "hybrid"], help="Discovery fingerprint type.")

    parser.add_argument(
        "--cluster-assignment-source",
        choices=["pred_cluster", "true_cluster"],
        default="pred_cluster",
        help="Cluster source for training. Use true_cluster only for oracle sanity checks.",
    )
    parser.add_argument(
        "--scheduler",
        default="random" if baseline else "balanced_cluster_round_robin",
        help="Training client scheduler.",
    )
    parser.add_argument(
...
def client_preparation_config_snapshot(cfg: dict) -> dict:
    """Keep client preparation artifacts focused on partition construction inputs."""
...
def modality_discovery_config_snapshot(cfg: dict) -> dict:
    """Keep modality discovery artifacts focused on partition and discovery inputs."""
    return _copy_selected_sections(
        cfg,
        [
            "experiment_name",
