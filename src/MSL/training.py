...
def _read_assignments(path: Path, assignment_column: str):
    if not path.exists():
        raise FileNotFoundError(f"Missing cluster assignments: {path}. Run modality discovery first.")
    rows = {}
...
def _load_checkpoint(path, server, clients, cluster_ids, cluster_to_slot, device):
    ...
    if saved_cluster_ids != [int(v) for v in cluster_ids]:
        raise ValueError("Checkpoint cluster_ids do not match the current training cluster IDs.")
    if saved_cluster_to_slot != {int(k): int(v) for k, v in cluster_to_slot.items()}:
        raise ValueError("Checkpoint cluster_to_slot does not match the current training mapping.")
    ...
    if set(client_states) != current_ids:
        raise ValueError("Checkpoint client encoder IDs do not match the current training clients.")
    for client in clients:
