...
from semantic_split_multimodal.data.datasets import _validate_subject_splits
...
def load_iemocap_dataset(cfg: dict, project_root: Path) -> dict:
    ...
    def build_split(session_ids):
        mask = torch.tensor([int(value) in session_ids for value in sessions.tolist()], dtype=torch.bool)
        if not bool(mask.any()):
            raise ValueError(f"IEMOCAP split produced no samples for sessions {sorted(session_ids)}.")
        return {
    ...
    train = build_split(session_splits["train"])
    validation = build_split(session_splits["validation"])
    test = build_split(session_splits["test"])
