...
def evaluate_naturally_paired_unpaired_baseline(
    server,
    clients_by_id,
        ...
        for batch in loader:
            *xs, yb = batch
            ...
            for modality_id, xb in enumerate(xs):
                ...
                client = clients_by_id[client_id]
                logits, _ = server.forward_cluster(cluster_id, client.encoder(xb.to(device)))
                logits_by_modality.append(logits)
            mean_logits = torch.stack(logits_by_modality, dim=0).mean(dim=0)
            pred = mean_logits.argmax(dim=1)
            y_true.extend(yb.tolist())
            y_pred.extend(pred.detach().cpu().tolist())
            total_loss += float(ce(mean_logits, yb.to(device)).item())
            total += int(yb.numel())
            num_eval_batches += 1
