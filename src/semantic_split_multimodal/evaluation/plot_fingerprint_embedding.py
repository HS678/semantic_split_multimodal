...
def write_fingerprint_pca_figure(
    ...
    os.environ.setdefault("MPLCONFIGDIR", str(Path(tempfile.gettempdir()) / "semantic_split_multimodal_matplotlib"))
    import matplotlib
...
def rebuild_fingerprint_figure(cfg: dict, stage1_dir: Path, stage2_dir: Path, device: torch.device):
    from semantic_split_multimodal.learning.models import create_client_encoder

    stage1_dir = Path(stage1_dir)
    stage2_dir = Path(stage2_dir)
    payloads = [torch.load(path, map_location="cpu") for path in sorted((stage1_dir / "train_clients").glob("client_*.pt"))]
    if not payloads:
        raise FileNotFoundError(f"No client payloads found under {stage1_dir / 'train_clients'}.")
    clients = [Client.from_payload(payload) for payload in payloads]
    encoders = {}
    for client in clients:
        saved = torch.load(stage2_dir / "pretrained_encoders" / f"{client.client_id}_encoder.pt", map_location="cpu")
    ...
    true_map = _read_assignments(stage2_dir / "true_cluster.csv", "true_cluster")
    pred_map = _read_assignments(stage2_dir / "pred_cluster.csv", "pred_cluster")
    names = {}
