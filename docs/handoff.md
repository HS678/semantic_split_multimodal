...
- 正式配置每 10 rounds naturally paired validation，由 validation macro-F1 选择 `best_model.pt`；训练结束恢复 best 后 test 一次，正式指标使用 `final_metrics.json`，`last_model.pt` 仅用于诊断。
- D2D 尚未实现。
