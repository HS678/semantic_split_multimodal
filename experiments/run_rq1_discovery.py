...
# 将 client assignment 写为 csv，供 RQ2 KMeans baseline 复用。
...
def run_one(dataset: str, fold: int | None, seed: int, method: str, results_root: Path) -> dict:
    ...
    if method == "adaptive_isodata":
        ...
        raise ValueError(f"Unsupported RQ1 method: {method}")
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Run RQ1 discovery on existing real Stage1/Stage2 artifacts.")
    add_common_run_args(parser)
    parser.add_argument("--method", choices=RQ1_METHODS, default="adaptive_isodata")
...
def main(argv=None):
    ...
    print(f"RQ1 {args.method} finished: status={result['status']} Q_hat={result['Q_hat']}")
